...
#uploading original file as reference
FN = sys.argv[1]
...
first_seq_in = sys.argv[2]

first_pair = []
pattern = re.compile(first_seq_in)
...
for num in arr_index :
    first_pair.append(position_num[num])



last_seq_in = sys.argv[3]
...
#generating new file
iter_num = sys.argv[4]
iter_residue_num = int(sys.argv[5])
temp_position = open("4j4l_round%s_%s.key"%(iter_num, iter_residue_num), "w")

#writing down new position restrain
temp_position.write("parameters /root/bin-linux/params/amber99.prm\nforcefield /root/bin-linux/params/amber99.prm\nCUDA-DEVICE 0\nverbose\n")

for line in data :
    out_line = [ele.replace("\n", "") for ele in line.split(' ') if ele != '']
    if not out_line[0].startswith('ATOM') :
        continue
    if out_line[1] in first_pair   :
        temp_position.write("restrain-position %s %s %s %s 1000.0"%(out_line[1], out_line[-3], out_line[-2], out_line[-1]))
        temp_position.write("\n")
    try :
        if out_line[1] in last_pair[:iter_residue_num+1 ] :
            temp_position.write("restrain-position %s %s %s %s 1000.0"%(out_line[1], out_line[-3], out_line[-2], out_line[-1]))
            temp_position.write("\n")
    except :
